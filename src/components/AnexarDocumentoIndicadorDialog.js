...
const AnexarDocumentoIndicadorDialog = ({ controleId, onClose, onSuccess, controleItem, categorias, projetos, tiposIndicador, subcategorias }) => {
  ...
  const [descricao, setDescricao] = useState(controleItem?.indicador || '');
  ...
  const uploadFile = async (e) => {
    ...
    try {
      ...
      // Criar vinculação específica para indicadores
      await criarVinculacaoIndicador(responseData.id);
  ...
  const vincularDocumento = async () => {
    ...
    try {
      ...
      // Criar vinculação específica para indicadores
      await criarVinculacaoIndicador(documentoSelecionado);
  ...
  // Função específica para criar vinculação na tabela de indicadores
  const criarVinculacaoIndicador = async (documentoId) => {
    try {
      // Obter a sessão atual do usuário
      const { data: { session } } = await supabase.auth.getSession();
      ...
      // Criar vinculação na tabela documento_controle_indicador_geral_rel
      ...
      if (error) {
        ...
        // Atualizar o flag tem_documento na tabela controle_indicador_geral
      ...
      console.error('Erro ao criar vinculação indicador:', error);
      throw error;
    }
  };
      ...
      <div className="bg-white p-6 rounded-lg max-w-lg w-full">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-bold">
            {step === 'escolha' && 'Anexar Documento ao Indicador'}
          ...
          <div>
            <div className="mb-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                <h3 className="font-medium text-gray-700 mb-2">Detalhes do Indicador</h3>
                ...
                <p><strong>Indicador:</strong> {controleItem?.indicador || 'N/A'}</p>
                <p><strong>Tipo de Indicador:</strong> {tiposIndicador[controleItem?.tipo_indicador] || 'N/A'}</p>
                <p><strong>Subcategoria:</strong> {subcategorias[controleItem?.subcategoria_id] || 'N/A'}</p>
                {controleItem?.prazo_entrega && (
...
export default AnexarDocumentoIndicadorDialog;