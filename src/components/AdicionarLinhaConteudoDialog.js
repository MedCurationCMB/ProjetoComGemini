...
const AdicionarLinhaConteudoDialog = ({ onClose, onSuccess, categorias, projetos }) => {
  ...
  const fetchLinhasBaseRecorrentes = async () => {
    try {
      ...
      // Buscar linhas de controle_conteudo com recorrência diferente de "sem recorrencia"
    ...
    try {
      ...
      if (isRecorrente) {
        ...
        const { data: ultimaLinha, error: ultimaLinhaError } = await supabase
          ...
          .select('*')
          .eq('id_controleconteudo', linhaBaseId)
        ...
        if (!novaLinha.projeto_id || !novaLinha.categoria_id || !novaLinha.prazo_entrega_inicial) {
        ...
        const dadosInsercao = {
          ...
          prazo_entrega_inicial: novaLinha.prazo_entrega_inicial,
                    ...
                    >
                      ...
                      {linhasBase.map((linha) => (
                        <option key={linha.id} value={linha.id}>
                          ID {linha.id} - {projetos[linha.projeto_id] || 'Projeto'} - {linha.descricao} ({linha.recorrencia})
                        </option>
                      ))}
                    </select>
                  )}
...
export default AdicionarLinhaConteudoDialog;