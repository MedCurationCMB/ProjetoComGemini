...
import { FiAward, FiZap, FiCpu, FiFileText } from 'react-icons/fi';

const MultiDocAnalysisDialog = ({ selectedDocuments, onClose, onAnalysisComplete }) => {
  ...
  const [docTexts, setDocTexts] = useState(null);
  const [preparingTexts, setPreparingTexts] = useState(false);
  ...
  // Buscar textos dos documentos selecionados
  const fetchDocumentTexts = async () => {
    ...
    try {
      ...
      if (!data || data.length === 0) {
        toast.error('Não foi possível encontrar os documentos selecionados');
        return null;
      }

      // Verificar se todos os documentos possuem texto
      const emptyDocs = data.filter(doc => !doc.conteudo || doc.conteudo.trim() === '');
      if (emptyDocs.length > 0) {
        toast.error(`${emptyDocs.length} documento(s) não possuem texto extraído. Todos os documentos devem ter texto.`);
        return null;
      }

      // Construir texto combinado com títulos para cada documento
      let combinedText = '';
      for (const doc of data) {
        combinedText += `\n\n### Texto Documento ID ${doc.id} (${doc.nome_arquivo}) ###\n\n`;
        combinedText += doc.conteudo;
      }
      ...
      console.error('Erro ao buscar textos dos documentos:', error);
      toast.error('Erro ao preparar os textos dos documentos selecionados');
      return null;
    } finally {
      setPreparingTexts(false);
    }
  };

  // Criar registro na tabela de análises
  const createAnalysisRecord = async (promptId, documentIds, combinedText) => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .insert([
          {
            prompt_id: promptId,
            documentos_selecionados: documentIds,
            texto_documentos_selecionados: combinedText,
  ...
  // Atualizar registro na tabela de análises com o resultado da IA
  ...
  const handleAnalyze = async () => {
    ...
    try {
      ...
      const textsResult = await fetchDocumentTexts();
      if (!textsResult) {
        setLoading(false);
        return;
      }
      
      setDocTexts(textsResult);
      ...
      <div className="bg-white p-6 rounded-lg max-w-3xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold flex items-center">
            <FiCpu className="mr-2 text-blue-600" /> Análise Múltipla com IA Gemini
          </h2>
          <button 
            onClick={onClose}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-blue-50 p-4 rounded-lg border border-blue-200 mb-4">
              <div className="flex items-start">
                <FiFileText className="text-blue-500 mt-1 mr-2 flex-shrink-0" />
                <div>
                  <h3 className="font-medium text-blue-800 mb-1">Documentos selecionados: {selectedDocuments.length}</h3>
                  <p className="text-sm text-blue-700">
                    Você selecionou {selectedDocuments.length} documento(s) para análise combinada. 
                    Ao clicar em "Analisar com IA", o texto de todos estes documentos será enviado para análise.
                  </p>
                </div>
              </div>
            </div>
            ...
            <div>
              <label htmlFor="promptSelect" className="block text-sm font-medium text-gray-700 mb-2 items-center">
            ...
            <div className="pt-4">
              <button
                ...
                disabled={loading || !selectedPromptId || chaveDisponivel === false || selectedDocuments.length === 0}
                className={`w-full flex justify-center items-center py-2 px-4 border border-transparent rounded-md shadow-sm text-white ${
                  loading || !selectedPromptId || chaveDisponivel === false || selectedDocuments.length === 0
              ...
              >
                {loading ? (
                  <>
                    ...
                    {preparingTexts ? 'Preparando documentos...' : 'Processando análise...'}
                  </>
                ) : (
                  <>
...
export default MultiDocAnalysisDialog;