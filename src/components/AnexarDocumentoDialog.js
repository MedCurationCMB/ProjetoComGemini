...
const AnexarDocumentoDialog = ({
  ...
  const vincularDocumento = async () => {
    ...
    try {
      ...
      // SEMPRE criar um novo vínculo, independente do modo (anexar ou substituir)
      // Este é o trecho que precisa ser corrigido
      ...
      toast.success(
        isSubstituicao
          ? "Documento substituído com sucesso!"
          : "Documento vinculado com sucesso!"
      );
  ...
  const documentosFiltrados = documentosExistentes.filter((doc) => {
    ...
    const textoCompleto =
      `${nomeArquivo} ${descricaoDoc} ${projetoNome} ${categoriaNome}`.toLowerCase();
            ...
            <div className="mb-4">
              <div className="bg-gray-50 p-4 rounded-lg">
                ...
                <p>
                  ...
                  {projetos[controleItem?.projeto_id] || "N/A"}
                </p>
                <p>
                  <strong>Categoria:</strong>{" "}
                  {categorias[controleItem?.categoria_id] || "N/A"}
                </p>
                <p>
                  <strong>Descrição:</strong> {controleItem?.descricao || "N/A"}
                </p>
                <p>
                  <strong>Tipo de Item:</strong>{" "}
                  {isGeralTable ? "Controle Geral" : "Controle Base"}
                </p>
                {isGeralTable && controleItem?.prazo_entrega && (
                  <p>
                    <strong>Prazo:</strong>{" "}
                    {new Date(controleItem.prazo_entrega).toLocaleDateString(
                      "pt-BR"
                    )}
                  </p>
                )}
                  ...
                  {documentosFiltrados.map((doc) => (
                    ...
                    >
                      <div className="flex items-start space-x-3">
                        <div
                          className={`flex-shrink-0 mt-1 h-4 w-4 rounded-full border ${
                        ...
                        <div className="flex-1">
                          ...
                          <div className="flex items-center text-xs text-gray-500 mt-1">
                            <span className="mr-2">
                              {categorias[doc.categoria_id] || "Categoria N/A"}
                            </span>
                            <span className="mr-2">•</span>
                            <span>
                              {projetos[doc.projeto_id] || "Projeto N/A"}
                            </span>
                          </div>
                          <p className="text-xs text-gray-500 mt-1">
