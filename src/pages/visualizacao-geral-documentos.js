...
export default function VisualizacaoGeralDocumentos({ user }) {
  ...
  const [kpis, setKpis] = useState({
    totalDocumentos: 0,
    documentosComArquivo: 0,
    documentosSemArquivo: 0
  });
  
  // Novos KPIs para documentos em atraso
  ...
  const gerarTextoDescritivo = () => {
    ...
    return `Documentos com prazo de entrega entre ${formatarData(dataInicio)} e ${formatarData(dataFim)}`;
  };

  // NOVA FUNÇÃO: Gerar texto descritivo dos documentos em atraso com lógica atualizada
  const gerarTextoDescritivoAtraso = () => {
    ...
    return `Documentos em atraso com base na data de referência: ${formatarData(dataReferencia)} (${tipoReferencia})`;
  };
  ...
  useEffect(() => {
    const fetchFiltrosDisponiveis = async () => {
      try {
        // Buscar projetos únicos que têm documentos
        ...
        // Buscar categorias únicas que têm documentos
  ...
  useEffect(() => {
    const fetchKPIs = async () => {
      try {
        ...
        // 1. Documentos com arquivo (tem_documento = true, no período filtrado)
        let queryComArquivo = supabase.from('controle_conteudo_geral')
          .select('*', { count: 'exact', head: true })
          .eq('tem_documento', true);
        
        queryComArquivo = construirQueryComFiltros(queryComArquivo);
        queryComArquivo = aplicarFiltrosData(queryComArquivo);
        
        const { count: documentosComArquivo, error: comArquivoError } = await queryComArquivo;
        if (comArquivoError) throw comArquivoError;

        // 2. Documentos sem arquivo (tem_documento = false, no período filtrado)
        let querySemArquivo = supabase.from('controle_conteudo_geral')
          .select('*', { count: 'exact', head: true })
          .eq('tem_documento', false);
        
        querySemArquivo = construirQueryComFiltros(querySemArquivo);
        querySemArquivo = aplicarFiltrosData(querySemArquivo);
        
        const { count: documentosSemArquivo, error: semArquivoError } = await querySemArquivo;
        if (semArquivoError) throw semArquivoError;

        // 3. NOVA LÓGICA: Total é a soma dos dois anteriores
        const totalDocumentos = (documentosComArquivo || 0) + (documentosSemArquivo || 0);

        // Atualizar estado
        setKpis({
          totalDocumentos: totalDocumentos,
          documentosComArquivo: documentosComArquivo || 0,
          documentosSemArquivo: documentosSemArquivo || 0
        });
  ...
  // Buscar dados dos KPIs de documentos em atraso
  useEffect(() => {
    const fetchKPIsAtraso = async () => {
      try {
        ...
        // 1. Todos os documentos em atraso (tem_documento = false e prazo_entrega < data_referencia)
        let queryTodosAtrasados = supabase.from('controle_conteudo_geral')
          .select('*', { count: 'exact', head: true })
          .eq('tem_documento', false)
        ...
        // 2. Documentos em atraso até 7 dias antes da data de referência
        let queryAtrasados7Dias = supabase.from('controle_conteudo_geral')
          .select('*', { count: 'exact', head: true })
          .eq('tem_documento', false)
          .lt('prazo_entrega', dataReferencia)
        ...
        // 3. Documentos em atraso até 30 dias antes da data de referência
        let queryAtrasados30Dias = supabase.from('controle_conteudo_geral')
          .select('*', { count: 'exact', head: true })
          .eq('tem_documento', false)
          .lt('prazo_entrega', dataReferencia)
        ...
        console.error('Erro ao buscar KPIs de documentos em atraso:', error);
        toast.error('Erro ao carregar dados de documentos em atraso');
      } finally {
  ...
  useEffect(() => {
    const fetchTabelas = async () => {
      try {
        ...
        // Construir query base para documentos vencidos usando a data de referência
        let queryDocumentos = supabase
          .from('controle_conteudo_geral')
        ...
        // Aplicar filtros de projeto e categoria
        queryDocumentos = construirQueryComFiltros(queryDocumentos);

        const { data: documentosVencidos, error: documentosError } = await queryDocumentos;

        if (documentosError) throw documentosError;

        // 2. Agrupar por projeto_id e categoria_id e contar (tabela combinada)
        const agrupamentoCombinado = {};
        documentosVencidos.forEach(item => {
        ...
        const projetoIds = [...new Set(documentosVencidos.map(item => item.projeto_id))];
        const categoriaIds = [...new Set(documentosVencidos.map(item => item.categoria_id))];
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      <Head>
        <title>Visualização Geral (Documentos)</title>
            ...
            {showFilters && (
              <div className="mb-4 p-4 bg-gray-50 rounded-lg">
                ...
                <div className="space-y-3">
                  ...
                  {/* ATUALIZADO: Filtro de Data de Referência para Documentos em Atraso */}
                  <div>
                    <label className="block text-xs font-medium text-gray-600 mb-1">
                      Data de Referência (Documentos em Atraso)
                    </label>
                    <input
                      type="date"
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        <div className="flex justify-between items-center mb-6">
          <div>
            <h1 className="text-2xl lg:text-3xl font-bold text-black">Visualização Geral (Documentos)</h1>
        ...
        {/* PRIMEIRA SEÇÃO DE KPIs - Documentos Gerais */}
        {loading ? (
            ...
            <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-green-500">
              ...
              <div className="mt-4">
                <p className="text-xs text-gray-500">
                  Documentos no período filtrado (hoje + 30 dias por padrão)
                </p>
              </div>
            </div>

            {/* KPI 2: Documentos Com Arquivo */}
            <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-blue-500">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-600">Com Arquivo Anexado</p>
                  <p className="text-3xl font-bold text-gray-900 mt-1">
                    {formatNumber(kpis.documentosComArquivo)}
                  </p>
                </div>
                <div className="flex-shrink-0">
              ...
              <div className="mt-4">
                <div className="flex items-center">
                  <p className="text-xs text-gray-500">
                    {calculatePercentage(kpis.documentosComArquivo, kpis.totalDocumentos)}% do total
                  </p>
                </div>
              </div>
            </div>

            {/* KPI 3: Documentos Sem Arquivo */}
            <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-yellow-500">
              <div className="flex items-center justify-between">
                <div className="flex-1">
                  <p className="text-sm font-medium text-gray-600">Sem Arquivo Anexado</p>
                  <p className="text-3xl font-bold text-gray-900 mt-1">
                    {formatNumber(kpis.documentosSemArquivo)}
                  </p>
                </div>
                <div className="flex-shrink-0">
              ...
              <div className="mt-4">
                <div className="flex items-center">
                  <p className="text-xs text-gray-500">
                    {calculatePercentage(kpis.documentosSemArquivo, kpis.totalDocumentos)}% do total
                  </p>
                </div>
              </div>
            </div>
        ...
        {/* NOVA SEÇÃO DE KPIs - Documentos em Atraso */}
        <div className="mb-8">
          <div className="mb-6">
            <h2 className="text-xl lg:text-2xl font-bold text-black">Documentos em Atraso</h2>
            <p className="text-gray-600 text-sm mt-1">
          ...
          {loadingAtraso ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {/* KPI 1: Todos os Documentos em Atraso */}
              <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-red-500">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    <p className="text-sm font-medium text-gray-600">Todos os Documentos</p>
                    <p className="text-3xl font-bold text-gray-900 mt-1">
                ...
                <div className="mt-4">
                  <p className="text-xs text-gray-500">
                    Total de documentos em atraso
                  </p>
                </div>
              </div>

              {/* KPI 2: Documentos em Atraso até 7 dias antes */}
              ...
              {/* KPI 3: Documentos em Atraso até 30 dias antes */}
        ...
        {/* NOVA SEÇÃO DE KPIs - Total Geral dos Documentos */}
        <div className="mb-8">
          <div className="mb-6">
            <h2 className="text-xl lg:text-2xl font-bold text-black">Total Geral dos Documentos</h2>
            <p className="text-gray-600 text-sm mt-1">
              Visão consolidada de todos os documentos do sistema
            </p>
          </div>
              ...
              <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-indigo-500">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-3xl font-bold text-gray-900 mt-1">
                      {formatNumber(kpis.totalDocumentos + kpisAtraso.todosAtrasados)}
                    </p>
                  </div>
                  <div className="flex-shrink-0">
                ...
                <div className="mt-4">
                  <div className="flex items-center space-x-4 text-xs text-gray-500">
                    <span className="flex items-center">
                      ...
                      Total período: {formatNumber(kpis.totalDocumentos)}
                    </span>
                    <span className="flex items-center">
              ...
              <div className="bg-white rounded-lg shadow-md p-6 border-l-4 border-amber-500">
                <div className="flex items-center justify-between">
                  <div className="flex-1">
                    ...
                    <p className="text-3xl font-bold text-gray-900 mt-1">
                      {formatNumber(kpis.documentosSemArquivo + kpisAtraso.todosAtrasados)}
                    </p>
                  </div>
                  <div className="flex-shrink-0">
                ...
                <div className="mt-4">
                  <div className="flex items-center space-x-4 text-xs text-gray-500">
                    <span className="flex items-center">
                      ...
                      Sem arquivo: {formatNumber(kpis.documentosSemArquivo)}
                    </span>
                    <span className="flex items-center">
                      <div className="w-2 h-2 bg-red-500 rounded-full mr-1"></div>
                      Atrasados: {formatNumber(kpisAtraso.todosAtrasados)}
          ...
          <div className="mt-8 grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                ...
                Projetos com Documentos em Atraso
                {hasFiltrosAtivos() && (
              ...
              {loadingTabelas ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  ...
                  <p>Nenhum projeto com documentos em atraso</p>
                </div>
              ) : (
            ...
            <div className="bg-white rounded-lg shadow-md p-6">
              <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
                ...
                Categorias com Documentos em Atraso
                {hasFiltrosAtivos() && (
                  <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">
                    Filtrado
                  </span>
              ...
              {loadingTabelas ? (
                ...
                <div className="text-center py-8 text-gray-500">
                  <FiTable className="h-12 w-12 mx-auto mb-4 text-gray-300" />
                  <p>Nenhuma categoria com documentos em atraso</p>
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="min-w-full divide-y divide-gray-200">
        ...
        <div className="mt-8 bg-white rounded-lg shadow-md p-6">
          <h2 className="text-lg font-semibold text-gray-900 mb-4 flex items-center">
            ...
            Detalhamento de Documentos em Atraso por Projeto e Categoria
            {hasFiltrosAtivos() && (
              <span className="ml-2 text-xs bg-blue-100 text-blue-800 px-2 py-1 rounded-full">
                Filtrado
              </span>
          ...
          {loadingTabelas ? (
            ...
            <div className="text-center py-8 text-gray-500">
              <FiTable className="h-12 w-12 mx-auto mb-4 text-gray-300" />
              <p>Nenhum documento em atraso encontrado</p>
            </div>
          ) : (
            <div className="overflow-x-auto">
              <table className="min-w-full divide-y divide-gray-200">
